...
class ApeSafe(Safe):
    ...
    def multisend_from_receipts(self, receipts: List[TransactionReceipt] = None, safe_nonce: int = None) -> SafeTx:
        ...
        data = MultiSend(self.ethereum_client).build_tx_data(txs)
    ...
    def sign_with_frame(self, safe_tx: SafeTx, frame_rpc="http://127.0.0.1:1248") -> bytes:
        ...
        eip712_structured_data = safe_tx.eip712_structured_data
        eip712_structured_data['message']['data'] = eip712_structured_data['message']['data'].hex()
        signature = frame.manager.request_blocking('eth_signTypedData_v4', [account, eip712_structured_data])
